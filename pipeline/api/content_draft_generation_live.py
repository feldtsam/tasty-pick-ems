...
def generate_content_drafts_for_pending(
    ...
    # REAL PROBLEM THIS CLOSES (confirmed in production, 2026-08-09): each
    # candidate is generated by its own fully independent Claude call, so
    # nothing previously told a later call in the same batch what an
    # earlier one already wrote — three Hot Hitters players all got
    # headlines like "[Name]'s swing is humming right now", and four Cold
    # Pitchers to Attack players facing the same real opposing pitcher all
    # got some variant of "cracks forming against Painter". Growing this
    # list across the loop and passing it into every subsequent call is
    # the fix: real variety instead of independent convergence on the same
    # phrasing when source facts happen to look similar. Titles only (not
    # why_reasons prose) — matches the real complaint, which was headline
    # repetition specifically; extend to why_reasons phrasing separately
    # if that turns out to need the same treatment.
    generated_titles = []
    ...
    for c in candidates:
        ...
        try:
            if writer_type == "tasty_six":
                draft = generate_tasty_six_draft(c, api_key, avoid_headlines=generated_titles)
                row = t6_draft_for_write(draft)
            else:
                draft = generate_shelf_card_draft(c, api_key, avoid_headlines=generated_titles)
