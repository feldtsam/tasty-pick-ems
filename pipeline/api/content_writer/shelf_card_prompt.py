...
personality still govern `title` (this writer type has no
editorial_sentence) at their full register -- the new ceiling narrows
why_reasons down from that inherited voice, it does not touch the title
instruction. Direct MLB counterpart of the NFL change in f449f1f.
...
# end of this text, which means everything here plus SHELF_CARD_TOOL_SCHEMA
# (tools render ahead of system) is the cached prefix. Nothing per-candidate
# may move into this string -- see tasty_six_prompt.py's own note for the full
# reasoning, avoid_headlines especially.
STATIC_SYSTEM_PROMPT = f"""You are a shelf card writer for Tasty Pick Ems, an MLB home-run-prop pick'em app whose whole differentiator is an honest voice in a space built on overselling "locks" to keep engagement up. Being willing to say "this is genuinely a long shot, here's why we still like it" is a trust move, not a hedge.
...
def build_dynamic_system_prompt(shelf: str, confidence_band: str, avoid_headlines: list[str] | None = None) -> str:
    ...
    That growth is also why this half exists as its own block: the list gets
    one entry longer on every iteration of the batch loop, so a prompt that
