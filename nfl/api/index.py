...
def curate_and_write_drafts_endpoint():
    ...
    TWO-CALL SPLIT (shelves_to_process / avoid_headlines / avoid_
    opening_phrases): a real, confirmed production incident, not a
    preventive change — fixing the shelf-card writer's real NaN-crash
    and max_tokens=1024 truncation bugs raised the real success rate of
    the per-row Claude calls, and a genuinely-completed generation takes
    LONGER than one that fails fast or gets cut off early. A real run
    at the (already-reduced) shelf_card_llm_top_n=8 then timed out at
    Vercel's 300s ceiling on a normal, not unlucky, invocation. These
    three optional params let ONE curation run be split across two HTTP
    calls, each handling a disjoint subset of the 7 primary shelves
    (curate_home_shelves.SHELF_SLUG's own 7 keys), each getting its own
    fresh 300s budget — see shape_content_draft_rows' own docstring for
    exactly what each scopes, and the "SUPERSEDE STALE APPROVED ROWS"
    block below for how the multi-run-duplication guard stays correct
    ...
    # TWO-CALL SPLIT (NFL Weekly Lifecycle, runtime ceiling fix) --
    # shelves_to_process/avoid_headlines/avoid_opening_phrases let ONE
    # curation run be split across two HTTP calls, each handling a
    # disjoint subset of the 7 primary shelves, each getting its own
    # fresh 300s ceiling -- see curate_home_shelves.shape_content_draft_
    # rows' own docstring for the real incident this closes (a genuinely-
    # completed generation takes LONGER than one that fails fast or gets
    # cut off early, so fixing the real success rate made the OLD single-
    # call architecture time out on a normal run, not an unlucky one).
    #
    # shelves_to_process (optional): a list of the real persisted shelf
    # SLUGS (e.g. "red_zone_trends", "attd_300_499" -- the same domain
    # nfl-content-drafts-write.ts's own VALID_SHELF_VALUES accepts, NOT
    # the internal Title-Case name), converted to the internal Title-
    # Case form via _SLUG_TO_SHELF before being threaded through to
    # curate_nfl_shelves. Omit it (the default) and every shelf is
    # processed in one call, unchanged from before this parameter
    # existed. Fails loud (400) on an unrecognized slug -- same "typo
    # should never silently resolve to an empty/wrong scope" reasoning
    # nfl-content-drafts-write.ts's own shelf validation already uses,
    # not a guess at what the caller meant.
    shelves_to_process_raw = data.get("shelves_to_process")
    shelves_to_process = None
    if shelves_to_process_raw is not None:
        if not isinstance(shelves_to_process_raw, list) or not shelves_to_process_raw:
            return jsonify({"error": "shelves_to_process must be a non-empty list of shelf slugs"}), 400
        unrecognized = [s for s in shelves_to_process_raw if s not in _SLUG_TO_SHELF]
        if unrecognized:
            return jsonify({
                "error": f"shelves_to_process contains unrecognized shelf slug(s): {unrecognized}",
                "hint": f"valid slugs: {sorted(_SLUG_TO_SHELF)}",
            }), 400
        shelves_to_process = [_SLUG_TO_SHELF[s] for s in shelves_to_process_raw]

    # avoid_headlines/avoid_opening_phrases (optional): the FIRST call's
    # own returned generated_titles/generated_opening_phrases, passed
    # back in on the SECOND call of a split so the real cross-batch
    # variety mechanism (see nfl_shelf_card_prompt.py) still sees titles/
    # opening phrases generated by call 1 as "already used" -- omit
    # (the default) and both lists start empty, exactly the original
    # single-call behavior.
    avoid_headlines_seed = data.get("avoid_headlines") or []
    avoid_opening_phrases_seed = data.get("avoid_opening_phrases") or []
    ...
    if preview_only:
        ...
        #
        #   shelves_to_process is a DIFFERENT kind of scoping and is
        #   deliberately NOT in the disabling condition above -- unlike
        #   player_ids_to_write/max_rows_to_write (an arbitrary test-only
        #   subset), a shelf-scoped call's own rows_to_write genuinely IS
        #   the complete, correct real population for exactly the shelves
        #   it processed (every real eligible player on those shelves is
        #   included, nothing missing) — so supersede stays correct here
        #   as long as the EXISTING rows it compares against are filtered
        #   to that same shelf scope too (below). Around the League rows
        #   (a division "shelf" is never a key in _SLUG_TO_SHELF) always
        #   pass through unfiltered, matching shape_content_draft_rows'
        #   own choice to leave them outside shelves_to_process entirely.
        if force and forward_result["success"] and not player_ids_to_write and not isinstance(max_rows_to_write, int):
            ...
            if not preflight_for_supersede["ok"]:
                ...
                existing_for_supersede = preflight_for_supersede["rows"]
                if shelves_to_process is not None:
                    scoped_slugs = set(shelves_to_process_raw)
                    existing_for_supersede = [
                        r for r in existing_for_supersede
                        if r.get("shelf") in scoped_slugs or r.get("shelf") not in _SLUG_TO_SHELF
                    ]
                stale_targets = compute_stale_approved_targets(rows_to_write, existing_for_supersede)
    ...
    return jsonify({
        ...
        # full accumulated cross-batch variety state -- a caller running
        # a second, disjoint-shelf call for the same (season, week) passes
        # these back in as that call's own avoid_headlines/avoid_opening_
        # phrases request fields, so the anti-repetition mechanism still
        # sees titles/phrases this call generated.
        "shelves_to_process": shelves_to_process_raw,
        "generated_titles": result["generated_titles"],
        "generated_opening_phrases": result["generated_opening_phrases"],
...
def curate_and_write_drafts_health_check():
    return jsonify({
        ...
        "usage": "POST {\"season\": int, \"week\": int, \"max_rows_to_write\": int (optional), "
                 ...
                 "\"shelves_to_process\": [str, ...] (optional, real shelf slugs e.g. "
                 "\"red_zone_trends\"), \"avoid_headlines\"/\"avoid_opening_phrases\": [str, ...] "
                 "(optional)}. Curates the given week's real stub-week data (home-shelf assignment, "
                 "Tasty Six, real content — including a real Claude call for Tasty Six rows) and "
                 "writes the result to nfl_content_drafts. Returns 409 status=locked (no curation "
                 "run) if any row for the week has already been reviewed; pass force:true to "
                 "override. max_rows_to_write/player_ids_to_write scope the actual write only; "
                 "curation always runs against the full real pool. shelves_to_process (two-call "
                 "split, for the real 300s-ceiling fix) scopes BOTH which shelves get a real Claude "
                 "call AND which shelves' rows are written this call — omit it to process all 7 "
                 "shelves in one call, unchanged; pass it twice with disjoint shelf sets, threading "
                 "the first call's own response.generated_titles/generated_opening_phrases back in "
                 "as the second call's avoid_headlines/avoid_opening_phrases, to split one curation "
                 "across two HTTP calls each with a fresh 300s budget.",
        "deployed_via": "github-auto-deploy",
    })
