...
# end of this text, which means everything here plus NFL_TASTY_SIX_TOOL_SCHEMA
# (tools render ahead of system) is the cached prefix. Nothing per-candidate
# may move into this string -- no shelf, no confidence band, no date, no
# player, and never avoid_headlines, which grows on every call within a batch
# and would therefore change the prefix on every call.
STATIC_SYSTEM_PROMPT = f"""You are the Tasty Six card writer for Tasty Pick Ems, an NFL anytime-touchdown (ATTD) pick'em app whose whole differentiator is an honest voice in a space built on overselling "locks" to keep engagement up. Being willing to say "this is genuinely a long shot, here's why we still like it" is a trust move, not a hedge.
...
def build_dynamic_system_prompt(shelf: str, confidence_band: str, avoid_headlines: list[str] | None = None) -> str:
    ...
    similar source facts) this closes. Its growth across the batch loop is
    also the single clearest reason this half is a separate block.
    """
