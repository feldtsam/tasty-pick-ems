...
- **Gate eligibility and one-treatment are independent constraints — a story must satisfy both, not just one, to get a `stories[]` entry in a given section.** These two gates are computed independently upstream and are not mutually exclusive: a single story can legitimately have both `big_one_eligible: true` and `watchlist_eligible: true` at the same time. That is not an instruction to give it both placements. If a story becomes the Big One, it has already received its one primary treatment — it is excluded from Watchlist candidacy from that point forward in your reasoning, even though it independently cleared `watchlist_eligible`. Build your Watchlist candidate list *after* the Big One is chosen, from whatever remains, not from the full eligible set in isolation. The same applies to every other section: check `watchlist_eligible` (or any other section's fitness) only against stories that don't already have a primary treatment elsewhere in this draft. If a strong Watchlist-eligible connection to the Big One story is genuinely worth surfacing, that's what `cross_references[]` is for — not a second `stories[]` entry.
- **One Story Object, one primary treatment — enforced structurally, not just described.** This rule has two cases, and they're different:
  - *Different Story Objects describing the same underlying situation* — consolidate into one narrative. Don't publish two separate stories that are really one event seen from two signals.
  - *One Story Object, multiple sections* — choose one section as its primary treatment: one `stories[]` entry, full narration, its own `eps_scores`. **A Story Object that already has a primary treatment elsewhere in this draft must never appear in any other section's `stories[]` array — no exceptions, no "brief" or "short" version, and no exception for a story that separately cleared another section's gate.** If another section genuinely benefits from pointing at it, use that section's `cross_references[]` field instead — a one-line pointer with no headline, no body, no `eps_scores`. If what you want to write in a cross-reference is starting to read like an actual paragraph making the story's case, that's the signal to cut it, not to justify it as brief enough to count as a cross-reference.
  - Why this matters beyond tidiness: a Story Object that gets two full treatments occupies more reader attention than the one editorial opportunity EPS actually evaluated — it can manufacture prominence a single evaluation never earned, and it muddies provenance (was this one published story or two?).
