# VENDORED, not pip-installed. See nfl/vendor/README.md for why.
#
# Verbatim copy of nfl_data_py==0.3.3's actual source (MIT license, see
# nfl/vendor/nfl_data_py/LICENSE) — the code itself is untouched. Only
# PyPI's own package METADATA declares nfl_data_py<2.0/<numpy 2.0, which
# is what's vendored around here, not this file's contents.
#
# Confirmed directly (2026-08): a plain `pip install pandas==2.2.3
# nfl_data_py==0.3.3` fails outright with pip's own ResolutionImpossible
# error — "nfl-data-py 0.3.3 depends on pandas<2.0 and >=1.0" — regardless
# of what else is in requirements.txt or what order it's listed in. This
# is what actually broke the Vercel build (nfl_data_py silently missing
# from the installed package set, because it was never a real line in
# requirements.txt in the first place — only documented as a manual local
# `pip install --no-deps` step, which a single `pip install -r
# requirements.txt` build never runs). A per-line --no-deps isn't a real
# pip requirements.txt feature, and betting on an unverified Vercel
# custom-install-command override felt worse than a fix that works
# identically on any pip/uv-based installer. This package's actual code
# has been validated against pandas 2.x throughout this entire project —
# only the declared metadata pin was ever stale, never real incompatible
# behavior — so vendoring the already-working copy directly, with its own
# accurate import path, sidesteps the conflict at the source instead of
# working around it at build time.
