...
**Scoring math changed from NFL in two places (2026-09, from the real-data weeks-1-8 validation):**
...
2. **Thin-sample rate gate.** `conversion_rate` + `recent_td_production` inputs are forced to neutral-50 until a player's cumulative RZ touches *through the prior week* reach `min_cumulative_rz_touches_for_rate` (=15, matching `min_rz_touches_for_qualification`). Kills the backup/walk-on/transfer who rides 3–6 touches and two recent TDs to the top of the board.
...
**⚠️ Open — the fixes above did not fully resolve the early-season distortion.** With `proven_heat` gated, sub-15-cumulative-touch players now surface via `emerging_heat` instead — a trend computed on 1–3 RZ touches/game still percentile-ranks high (e.g. a 1-touch/0-TD back at `td_opportunity` ~86 in week 8). `completeness` flags these (20 vs a real workhorse's 90) but a sort-by-score doesn't. **Decision pending:** extend the same cumulative-touch gate to the `emerging_heat` trend inputs (full Role-&-Momentum-style "contributes nothing until you've earned a sample"), vs. a completeness-column-only approach. `corr(td_opportunity, rz_touches)` is still only ~+0.1 mid-season.
