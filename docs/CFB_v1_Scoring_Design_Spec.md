...
- `/plays` has no `gameId` filter, and its `playType` query param wants an *abbreviation* (the full text value `"Rushing Touchdown"` returns 0 rows — confirmed on the live API). So `/plays` is pulled **once per team that played a completed game** (`/plays?year=&week=&team=&classification=fbs`, ~150–180 rows each, cap-safe), deduped on play `id`, then filtered in memory. For an 8-game smoke slate that is ~16 calls; a full ~65-game FBS week is ~130 `/plays` calls on top of the ~65 `/plays/stats`. If that pushes the free-tier monthly budget, the follow-up is to resolve the two TD play-type *abbreviations* from `/plays/types` and switch to 2 `playType`-filtered calls per week — deferred, not a v1 blocker.
