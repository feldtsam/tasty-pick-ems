...
- **CFBD `/plays` quirk 3 — two rows per touchdown.** For every rushing/passing TD, `/plays` returns *two* entries with different `id`s, both tagged `playType="… Touchdown"`: a **play-by-play** row (`"(08:38) #20 M.Ford rush left for 2 yards … HAW00 TOUCHDOWN"`) and a **scoring-summary** row (`"Micah Ford 2 Yd Run (Emmet Kenney Kick)"`). Only the *scoring-summary* `id` is present in `/plays/stats` — and it carries the scorer's `Rush`/`Reception` row (and, for ~a few plays a week, a `statType="Touchdown"` row too, though those are far too sparse — 3 in the 8-game smoke week — to use as the TD source). Intersecting the raw `/plays` TD-id set with `/plays/stats`' own `playId`s drops the play-by-play duplicates and keeps exactly the ids a touch can be attributed to. Post-intersection the `td_attribution` diagnostic's `unmatched` (a scoring-summary id in `/plays/stats` with no rush/reception on it) should be ~0; `pbp_duplicates_dropped` is expected and benign (~half the raw set).
...
**`td_attribution` diagnostic** in the ingestion response reports `td_plays_from_plays_raw`, `td_plays_in_play_stats`, `pbp_duplicates_dropped` (benign — ~half the raw set), `credited`, and `in_play_stats_but_no_touch` / `unmatched` (the number to watch — a scoring-summary id in `/plays/stats` with no rush/reception on it). Post-fix, `unmatched` runs ~0–1 (a single passing TD whose scoring-summary id carries a stat row but no `Reception`). A persistent higher count means the join has drifted and needs re-checking against live data.

### 8b. Full-week call volume & latency — added 2026-08-31

A full FBS week is ~90 completed games. The run makes **~94 CFBD calls**: `/games` ×1, `/roster?classification=fbs` ×1, `/plays/types` ×1, `/plays?playType=TD` ×1, and `/plays/stats?gameId=` ×~90. The `/plays/stats` fan-out is fetched **concurrently** (thread pool, 8 workers) — measured ~0.32s/call serial, so the fan-out is ~4s parallel instead of ~29s. Measured/estimated full-week wall-clock ≈ **15s** (fixed calls ~5s + `/plays/stats` ~4s + `/roster` ~3.5s + aggregation ~3s). Vercel plan is **Pro** (`maxDuration` ceiling 300s); `cfb/vercel.json` is set to **120s** as headroom.

The ingestion response always carries a `timing` block (per-phase wall-clock) and a `cost_estimate` block (`estimate_week_cost()` — projected call count + wall-clock for the slate, with a `high_call_volume` flag at >150 calls). `POST {"dry_run": true}` returns just the estimate without ingesting. `cfb/test_plays_stats.py` asserts a 90-game week stays well under the timeout and that a pathological game count trips the tripwire — a guard against silent regression as game/roster counts grow.
